...
describe("Event API V1 Suite", () => {
  ...
  it("should return an array of paginated events default pageSize is 5", async () => {
    ...
    const response = await chai.request(server).get(`${BASE_PATH}/events`);
    ...
    assert.exists(
      events[0].id,
      "ID should be exists in first element in events array!"
    );
    //   assert.exists(events.user_id, 'User ID should be exists');
    assert.exists(
      events[0].name,
      "Events name should be exists in first element"
    );
    assert.exists(
      events[0].location,
      "Events location should be exists in first element"
    );
    assert.exists(
      events[0].date,
      "Events date should be exists in first element"
    );
  });
  ...
  it("should update an existing event", async () => {
    ...
    assert.equal(
      updatedEvent.name,
      updateEventInput.name,
      "Given name should be same!"
    );
    assert.equal(
      updatedEvent.location,
      updateEventInput.location,
      "Given name should be same!"
    );
  });
